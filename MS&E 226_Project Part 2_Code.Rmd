...
summary_ols <- summary(ols_train)

# Extract coefficients table
coefficients_table <- summary_ols$coefficients

# Identify statistically significant coefficients
significant_coefficients <- coefficients_table[coefficients_table[, 4] < 0.05, ]

significant_coefficients_df <- data.frame(
  Variable = rownames(significant_coefficients),
  Pr_greater_than_t = significant_coefficients[, 4]
)
...
subset_column_names <- c("Demand.Summer.Peak", "Demand.Winter.Peak", "Sources.Generation", 
                         "Utility.Region", "Max.Revenue.Sector", "Revenue.Total", "Utility.Type")
df_subset <- df[, subset_column_names]
df_subset$Log_Revenue_Total <- log1p(df_subset$Revenue.Total)

# Create the formula for the subset model
subset_formula <- as.formula("Log_Revenue_Total ~ Demand.Summer.Peak + Demand.Winter.Peak + 
                              Sources.Generation + Utility.Region + Max.Revenue.Sector + Utility.Type")

# Fit the subset OLS model
subset_ols <- lm(subset_formula, data = df_subset)

# Print the summary of the subset model
summary(subset_ols)
...
```{r}
# Extract significant coefficients from the subset model
subset_summary <- summary(subset_ols)
subset_coefficients <- subset_summary$coefficients
significant_subset_coefficients <- subset_coefficients[subset_coefficients[, 4] < 0.05, ]

significant_subset_coefficients_df <- data.frame(
  Variable = rownames(significant_subset_coefficients),
  Pr_greater_than_t = significant_subset_coefficients[, 4]
)

# Print the table
print(significant_subset_coefficients_df)
```
...
comparison_pvalues <- comparison_pvalues[order(comparison_pvalues$Training_Pvalues), ]

# Set options to avoid scientific notation
options(scipen = 999)

# Display the comparison of p-values
print("Comparison of p-values from training and test data:")
print(comparison_pvalues)

# Extract coefficients from the training and test models
training_coefs <- coef(ols_train)
holdout_coefs <- coef(ols_test)

# Ensure feature names match between training and holdout coefficients
common_features <- intersect(names(training_coefs), names(holdout_coefs))

# Subset coefficients to only include common features
training_coefs <- training_coefs[common_features]
holdout_coefs <- holdout_coefs[common_features]
...
comparison_coefs <- comparison_coefs[order(-comparison_coefs$Training_Coefs), ]

# Display the comparison of coefficients
