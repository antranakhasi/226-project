...
knn_numeric_cols <- sapply(X, is.numeric)
knn_categorical_cols <- sapply(X, function(x) is.factor(x) | is.character(x))

# Create dummy variables for categorical features
dummy <- dummyVars(" ~ .", data = X[, knn_categorical_cols])
categorical_dummy <- predict(dummy, newdata = X[, knn_categorical_cols])

# Combine numeric and dummy-coded categorical variables
X <- cbind(X[, knn_numeric_cols], categorical_dummy)
...
knn_class_final_model <- knn(train = X, test = X, cl = y, k = best_k[[1]])
...
lr_formula <- as.formula("Uses.Consumed.Binary ~ Utility.Type + Max.Revenue.Sector + Demand.Winter.Peak + Demand.Summer.Peak + Revenue.Retail + Revenue.Retail:Max.Revenue.Sector")
...
lr_class_final_model <- glmnet(
  as.matrix(df_clean_scaled[, all.vars(lr_formula)[-1]]), 
...
# Get the best model
dt_class_final_model <- model_grid$finalModel
...
```{r Regression prediction with winning modeling strategy, Random Forest Model}
...
# Select the relevant columns in the test set
holdout_pred <- holdout[, column_names]
# Apply the same transformation to the test set
holdout_pred$Log_Revenue_Total <- log1p(holdout$Revenue.Total)

# Predict on the test set
rf_test_predictions <- predict(rf_model, newdata = holdout_pred)

# Check for NA values in predictions or Log_Revenue_Total
if (any(is.na(rf_test_predictions)) || any(is.na(holdout_pred$Log_Revenue_Total))) {
    stop("Predictions or Log_Revenue_Total contain NA values.")
}

# Calculate RMSE
rf_test_rmse <- sqrt(mean((rf_test_predictions - holdout_pred$Log_Revenue_Total)^2))
cat(paste("Random Forest Test RMSE:", rf_test_rmse, "\n"))
...
In part 1, the logistic regression classifier had the best performance. This is the classifier that will be used for prediction on the holdout set.

```{r Classification prediction with winning modeling strategy}
# Select specific columns from the dataframe based on positive correlation > 0.1 or negative correlation <-0.1
holdout_clean <- holdout[, c("Uses.Consumed.Binary", # Binary outcome variable
                   "Utility.Type", # Positively or negatively correlated with outcome, depending on factor level 
                   "Demand.Winter.Peak", # Positively correlated with outcome
                   "Demand.Summer.Peak", # Positively correlated with outcome
                   "Max.Revenue.Sector", # Negatively correlated with outcome
...
# Loop through each column in the dataset and apply the factor transformation to each non-numeric (categorical) variable
holdout_clean[] <- lapply(holdout_clean, function(x) {
  if (!is.numeric(x)) {
    factor(x, levels = unique(x))
  } else {
    x
...
holdout_clean$Uses.Consumed.Binary <- factor(holdout_clean$Uses.Consumed.Binary, levels = c("0", "1"))
levels(holdout_clean$Uses.Consumed.Binary) <- make.names(levels(holdout_clean$Uses.Consumed.Binary))


# Scale numeric columns (use same scaling parameters as training data)
holdout_clean_scaled <- holdout_clean
numeric_cols_holdout <- sapply(holdout_clean, is.numeric)  # Identify numeric columns
holdout_clean_scaled[numeric_cols_holdout] <- scale(
  holdout_clean[numeric_cols_holdout], 
  center = attr(scale(df_clean[numeric_cols]), "scaled:center"), 
  scale = attr(scale(df_clean[numeric_cols]), "scaled:scale")
)

# Subset the holdout dataset to include only variables in the formula
required_columns <- all.vars(formula)[-1]  # Exclude the response variable
holdout_clean <- holdout[, required_columns, drop = FALSE]  # Keep only required columns


# subset only covariates in the formula 
formula_vars <- all.vars(lr_formula)

# Subset the dataframe to include only the columns in the formula
holdout_clean_scaled <- holdout_clean_scaled[, formula_vars, drop = FALSE]

# Separate outcome and covarites

Y_holdout <- holdout_clean_scaled$Uses.Consumed.Binary

X_holdout <- holdout_clean_scaled[, colnames(holdout_clean_scaled) != "Uses.Consumed.Binary"]

# Convert to matrix for glmnet predictions
holdout_class_matrix <- as.matrix(X_holdout)


# Make predictions (probabilities)
predictions_prob <- predict(
  lr_class_final_model, 
  newx = holdout_class_matrix, 
  s = model_cv$bestTune$lambda, 
  type = "response"
)
...
# Calculate 0-1 loss
true_labels <- holdout$Uses.Consumed.Binary  # True labels from holdout
misclassified <- sum(predictions_binary != true_labels)
zero_one_loss_holdout <- misclassified / length(true_labels)

# Print results
print(paste("0-1 Loss on holdout data:", round(zero_one_loss_holdout, 4)))
...
```{r create OLS model using covariates selectedin Lasso}

# Generate the final Lasso model 

# Extract predictors (X) and response variable (y) for the train set
X_train <- model.matrix(Log_Revenue_Total ~ ., data = df_pred)[, -1]
y_train <- df_pred$Log_Revenue_Total

# Refit the Lasso model on the test set using the final lambda from training
final_lasso_model <- glmnet(X_train, y_train, alpha = 1, lambda = final_lambda_lasso, standardize = TRUE)


# Extract non-zero coefficients from the final Lasso model
lasso_coefficients <- coef(final_lasso_model)
lasso_coefficients <- as.matrix(lasso_coefficients)  # Convert to a matrix
nonzero_covariates <- rownames(lasso_coefficients)[lasso_coefficients != 0]
nonzero_covariates <- nonzero_covariates[nonzero_covariates != "(Intercept)"] # Exclude intercept

# Identify all one-hot encoded categorical variables in this list and remove all but one
remove_after_capital <- function(x) {
  # Use a regular expression to find a lowercase letter followed by an uppercase letter
  gsub("([a-z])[A-Z].*", "\\1", x)
}

# Apply the function to the vector of strings
unique_nonzero_covariates <- sapply(nonzero_covariates, remove_after_capital)

# Print the modified strings
print(unique_nonzero_covariates)

# Identify unique strings (appear exactly once)
unique_nonzero_covariates <- names(table(unique_nonzero_covariates)[table(unique_nonzero_covariates) == 1])

# Print the unique strings
print(unique_nonzero_covariates)

# Subset the dataframe with the selected covariates
df_selected <- df_pred[, c("Log_Revenue_Total", unique_nonzero_covariates)]

# Create the formula for OLS regression
ols_formula <- as.formula(paste("Log_Revenue_Total ~", paste(unique_nonzero_covariates, collapse = " + ")))

# Run the OLS regression
ols_train <- lm(ols_formula, data = df_selected)
...
#### (a)
```{r 2a}
# Extract the final Ridge model using the best lambda
final_ridge_model <- ridge_model$finalModel

# Extract coefficients from the final model
coefficients <- coef(final_ridge_model, s = final_lambda_ridge)
...
subset_column_names <- c(  "Demand.Summer.Peak", "Demand.Winter.Peak", "Sources.Generation", "Utility.Region", "Max.Revenue.Sector", "Revenue.Total", "Utility.Type")

# Create a new data frame with the subset of columns
df_subset <- df[, subset_column_names]

df_subset$Log_Revenue_Total <- log1p(df_subset$Revenue.Total)

# Train the Ridge Regression model using the subset of covariates
ridge_subset_model <- train(Log_Revenue_Total ~ ., 
                            data = df_subset, 
                            method = "glmnet",  
                            trControl = train_control,
                            metric = "RMSE", 
...
```
```{r}
# Get the coefficients for the best model at the selected lambda
best_lambda <- ridge_model$bestTune$lambda
best_model_coeffs <- coef(ridge_model$finalModel, s = best_lambda)

# Print all coefficients for the best Ridge Regression model
cat("All coefficients for the best Ridge Regression model:\n")
print(best_model_coeffs)

# Get coefficients for the subset model
best_lambda_subset <- ridge_subset_model$bestTune$lambda
best_model_coeffs_subset <- coef(ridge_subset_model$finalModel, s = best_lambda_subset)

# Print all coefficients for the subset model
cat("All coefficients for the Ridge Regression model with a subset of covariates:\n")
print(best_model_coeffs_subset)
```
...
# Save a copy of the holdout dataset to be transformed/cleaned 
# Create df_test with only the specified columns from holdout
df_test <- holdout[, column_names]

# Log-transform the Revenue.Total column in the test set to match training
df_test$Log_Revenue_Total <- log1p(df_test$Revenue.Total)

# Refit the OLS model on the test set 
ols_test <- lm(ols_formula, data = df_test)

# Extract p-values from the training and test models
training_pvalues <- summary(ols_train)$coefficients[, "Pr(>|t|)"]
test_pvalues <- summary(ols_test)$coefficients[, "Pr(>|t|)"]

# Remove the intercept term from p-values
training_pvalues <- training_pvalues[names(training_pvalues) != "(Intercept)"]
test_pvalues <- test_pvalues[names(test_pvalues) != "(Intercept)"]

# Ensure feature names match between the training and holdout p-values
common_features <- intersect(names(training_pvalues), names(test_pvalues))

# Subset p-values to only include common features
training_pvalues <- training_pvalues[common_features]
test_pvalues <- test_pvalues[common_features]

# Combine p-values into a comparison data frame
comparison <- data.frame(
  Predictor = common_features,
  Training_Pvalues = round(training_pvalues, 8),
  Test_Pvalues = round(test_pvalues, 8)
)

# Sort the comparison data frame in increasing order of Training_Pvalues
comparison <- comparison[order(comparison$Training_Pvalues), ]

# Set options to avoid scientific notation
options(scipen = 999)

# Display the comparison
print("Comparison of p-values from training and test data:")
print(comparison)


## compare coefficients

# Subset coefficients to only include common features
training_coefs <- training_coefs[common_features, , drop = FALSE]
holdout_coefs <- holdout_coefs[common_features, , drop = FALSE]

# Combine coefficients into a comparison data frame
comparison <- data.frame(
  Predictor = common_features,
  Training_Coefs = round(as.numeric(training_coefs), 8),
  Holdout_Coefs = round(as.numeric(holdout_coefs), 8)
)

# Sort the comparison data frame in decreasing order of Training_Coefs
comparison <- comparison[order(-comparison$Training_Coefs), ]

# Display the comparison
print("Comparison of coefficients from training and holdout data:")
print(comparison)
...
# Function to fit OLS on bootstrap sample and return coefficients
bootstrap_ols <- function(data, indices) {
  # Bootstrap sampling
  boot_sample <- data[indices, ]
  
  # Fit OLS model
  ols_fit <- lm(ols_formula, data = boot_sample)
  
  # Extract coefficients
  coefs <- coef(ols_fit)
  
  # Return coefficients (excluding intercept if not needed)
  return(coefs)
}
...
bootstrap_cis <- apply(bootstrap_results$t, 2, function(coefs) {
  quantile(coefs, probs = c(0.025, 0.975))
})

# Format as a data frame
cis_df <- data.frame(
  Predictor = names(bootstrap_results$t0),
  CI_Lower = bootstrap_cis[1, ],
  CI_Upper = bootstrap_cis[2, ]
)


# Extract OLS confidence intervals from the standard regression output
ols_cis_standardized <- confint(ols_test, level = 0.95)
ols_cis_df_standardized <- data.frame(
  Predictor = rownames(ols_cis_standardized),
  OLS_CI_Lower_Standardized = ols_cis_standardized[, 1],
  OLS_CI_Upper_Standardized = ols_cis_standardized[, 2]
)

# Merge with Bootstrap confidence intervals for comparison
comparison_ols <- merge(
  cis_df,  # Bootstrap confidence intervals from OLS
  ols_cis_df_standardized,
  by = "Predictor",
  all.x = TRUE
)

# Display the comparison
print("Comparison of Bootstrap and Standardized OLS Confidence Intervals:")
print(comparison_ols)

# Visualize the difference in Bootstrap vs. Standardized OLS CIs

# Calculate confidence interval sizes for Bootstrap and OLS
comparison_ols$Bootstrap_CI_Size <- comparison_ols$CI_Upper - comparison_ols$CI_Lower
comparison_ols$OLS_CI_Size <- comparison_ols$OLS_CI_Upper_Standardized - comparison_ols$OLS_CI_Lower_Standardized

# Reshape the data for plotting
ci_data_ols <- comparison_ols %>%
  select(Predictor, Bootstrap_CI_Size, OLS_CI_Size) %>%
  pivot_longer(cols = c(Bootstrap_CI_Size, OLS_CI_Size), 
               names_to = "Method", 
               values_to = "CI_Size")

# Rename methods for better readability
ci_data_ols$Method <- ifelse(ci_data_ols$Method == "Bootstrap_CI_Size", "Bootstrap OLS", "Standard OLS Output")

# Create the density plot
ggplot(ci_data_ols, aes(x = CI_Size, fill = Method)) +
  geom_density(alpha = 0.5) +
  scale_x_log10() +
  labs(
    title = "Density Plot of Confidence Interval Sizes: Bootstrap vs. Standard OLS Output",
    x = "Confidence Interval Size",
    y = "Density"
  ) +
  theme_minimal() +
  theme(
...
# Violin plot
ggplot(ci_data_ols, aes(x = Method, y = CI_Size)) +
  geom_violin(fill = "lightblue") +
  geom_boxplot(width = 0.1, fill = "white") +
  theme_minimal() +
  labs(title = "Distribution of CI_Size by Group", x = "Method", y = "CI_Size")
...
```{r Benjamini_Hochberg Procedure}

# Extract the p-values for the coefficients
p_values <- summary(ols_test)$coefficients[, 4] # 4th column contains p-values
...
results <- data.frame(
  Coefficient = rownames(summary(ols_test)$coefficients),
  P_Value = p_values,
...
# Optional: Identify which hypotheses are significant after FDR correction
significant <- results$Adjusted_P_Value < 0.05
results$Significant <- ifelse(significant, "Yes", "No")

# Display the results with significance labels
print(results)
...
```{r 2c Fit to test data Ridge}

# manually refit final ridge model 

# Refit the model manually using glmnet
X_train <- model.matrix(Log_Revenue_Total ~ ., data = df_pred)[, -1]
y_train <- df_pred$Log_Revenue_Total

final_ridge_model <- glmnet(X_train, y_train, alpha = 0, lambda = final_lambda_ridge, standardize = TRUE)
...
# Extract predictors (X) and response variable (y) for the test set
X_test <- model.matrix(Log_Revenue_Total ~ ., data = df_test)[, -1]
y_test <- df_test$Log_Revenue_Total

# Refit the ridge model on the test set using the final lambda from training
ridge_test <- glmnet(X_test, y_test, alpha = 0, lambda = final_lambda_ridge, standardize = TRUE)

# Extract coefficients from the test set
ridge_test_coefs <- coef(ridge_test)

# Extract coefficients from training and holdout models
training_coefs <- as.matrix(coef(final_ridge_model, s = final_lambda_ridge))
holdout_coefs <- as.matrix(ridge_test_coefs)

# Remove the intercept term (row with name "(Intercept)") from both coefficient matrices
training_coefs <- training_coefs[rownames(training_coefs) != "(Intercept)", , drop = FALSE]
holdout_coefs <- holdout_coefs[rownames(holdout_coefs) != "(Intercept)", , drop = FALSE]

# Ensure feature names match between the training and holdout coefficients
common_features <- intersect(rownames(training_coefs), rownames(holdout_coefs))
...
```{r 2d bootstrap Ridge}

# Function to fit Ridge on bootstrap sample and return coefficients
bootstrap_ridge <- function(data, indices, lambda) {
  # Bootstrap sampling
  boot_sample <- data[indices, ]
  
  # Create the model matrix from the bootstrap sample
  X_boot <- model.matrix(Log_Revenue_Total ~ ., data = boot_sample)[, -1]
  y_boot <- boot_sample$Log_Revenue_Total
  
  # Ensure that X_boot has the same column names as the original data
  full_X <- model.matrix(Log_Revenue_Total ~ ., data = data)[, -1]
  full_coefs <- numeric(ncol(full_X))  # Create a vector for all predictors (set to 0 initially)
  names(full_coefs) <- colnames(full_X)  # Name it with the original predictor names
  
  # Fit Ridge model
  ridge_fit <- glmnet(X_boot, y_boot, alpha = 0, lambda = lambda, standardize = TRUE)
  
  # Extract coefficients (excluding intercept)
  coefs <- as.vector(coef(ridge_fit)[-1])  # Exclude intercept
  coef_names <- colnames(X_boot)  # Names of predictors in this bootstrap sample
  
  # Assign the coefficients to the correct positions
  full_coefs[coef_names] <- coefs
...
bootstrap_results <- boot(
  data = df_pred,
  statistic = bootstrap_ridge,
  R = n_boot,
  lambda = final_lambda_ridge
)
...
## Compare to the confidence intervals computed in the standard regression output

# Identify numeric predictors
numeric_predictors <- sapply(df_pred[column_names], is.numeric)

# Standardize only numeric predictors
df_pred_standardized <- df_pred
df_pred_standardized[names(numeric_predictors)[numeric_predictors]] <- scale(
  df_pred[names(numeric_predictors)[numeric_predictors]]
)

# # See that non-numeric columns remain unchanged
# print("Standardized dataset:")
# print(head(df_pred_standardized))

# Fit OLS regression with standardized predictors
ols_model_standardized <- lm(Log_Revenue_Total ~ ., data = df_pred_standardized)

# Extract OLS confidence intervals
ols_cis_standardized <- confint(ols_model_standardized, level = 0.95)
ols_cis_df_standardized <- data.frame(
  Predictor = rownames(ols_cis_standardized),
  OLS_CI_Lower_Standardized = ols_cis_standardized[, 1],
  OLS_CI_Upper_Standardized = ols_cis_standardized[, 2]
...
# Merge with Bootstrap confidence intervals for comparison
comparison_ridge_OLS <- merge(
  cis_df,  # Bootstrap confidence intervals from Ridge
  ols_cis_df_standardized,
  by = "Predictor",
  all.x = TRUE
)
...
# Display the comparison
print("Comparison of Bootstrap and Standardized OLS Confidence Intervals:")
print(comparison_ridge_OLS)


# visualize the difference in Ridge vs. OLS CIs

# Calculate confidence interval sizes for Ridge and OLS
comparison_ridge_OLS$Ridge_CI_Size <- comparison_ridge_OLS$CI_Upper - comparison_ridge_OLS$CI_Lower
comparison_ridge_OLS$OLS_CI_Size <- comparison_ridge_OLS$OLS_CI_Upper_Standardized - comparison_ridge_OLS$OLS_CI_Lower_Standardized

# Reshape the data for plotting
ci_data_ridge <- comparison_ridge_OLS %>%
  select(Predictor, Ridge_CI_Size, OLS_CI_Size) %>%
  pivot_longer(cols = c(Ridge_CI_Size, OLS_CI_Size), 
               names_to = "Method", 
               values_to = "CI_Size")

# Rename methods for better readability
ci_data_ridge$Method <- ifelse(ci_data_ridge$Method == "Ridge_CI_Size", "Ridge", "OLS")

# Create the density plot
ggplot(ci_data_ridge, aes(x = CI_Size, fill = Method)) +
  geom_density(alpha = 0.5) +
  scale_x_log10() +
  labs(
    title = "Density Plot of Confidence Interval Sizes: Ridge vs. OLS",
    x = "Confidence Interval Size",
    y = "Density"
  ) +
  theme_minimal() +
...
# violin plot
ggplot(ci_data_ridge, aes(x = Method, y = CI_Size)) +
  geom_violin(fill = "lightblue") +
  geom_boxplot(width = 0.1, fill = "white") +
  theme_minimal() +
  labs(title = "Distribution of CI_Size by Group", x = "Group", y = "CI_Size")



```

```{r 2c Fit to test data Lasso}
...
# Refit the Lasso model on the test set using the final lambda from training
lasso_test <- glmnet(X_test, y_test, alpha = 1, lambda = final_lambda_lasso, standardize = TRUE)

# Extract coefficients from the test set
lasso_test_coefs <- coef(lasso_test)

# Extract coefficients from training and holdout models
training_coefs <- as.matrix(coef(lasso_model$finalModel, s = final_lambda_lasso))
holdout_coefs <- as.matrix(lasso_test_coefs)
...
```{r 2d bootstrap Lasso}

# Function to fit Lasso on bootstrap sample and return coefficients
bootstrap_lasso <- function(data, indices, lambda) {
  # Bootstrap sampling
  boot_sample <- data[indices, ]
  ...
  # Fit Lasso model
  lasso_fit <- glmnet(X_boot, y_boot, alpha = 1, lambda = lambda, standardize = TRUE)
  
  # Extract coefficients (excluding intercept)
  coefs <- as.vector(coef(lasso_fit)[-1])  # Exclude intercept
  coef_names <- colnames(X_boot)  # Names of predictors in this bootstrap sample
...
bootstrap_results <- boot(
  data = df_pred,
  statistic = bootstrap_lasso,
  R = n_boot,
  lambda = final_lambda_lasso
)
...
# Merge with Bootstrap confidence intervals for comparison
comparison_lasso_OLS <- merge(
  cis_df,  # Bootstrap confidence intervals from Lasso
  ols_cis_df_standardized,
  by = "Predictor",
  all.x = TRUE
)
...
# Display the comparison
print("Comparison of Bootstrap and Standardized OLS Confidence Intervals:")
print(comparison_lasso_OLS)


# visualize the difference in Lasso vs. OLS CIs

# Calculate confidence interval sizes for Lasso and OLS
comparison_lasso_OLS$Lasso_CI_Size <- comparison_lasso_OLS$CI_Upper - comparison_lasso_OLS$CI_Lower
comparison_lasso_OLS$OLS_CI_Size <- comparison_lasso_OLS$OLS_CI_Upper_Standardized - comparison_lasso_OLS$OLS_CI_Lower_Standardized

# Reshape the data for plotting
ci_data_lasso <- comparison_lasso_OLS %>%
  select(Predictor, Lasso_CI_Size, OLS_CI_Size) %>%
  pivot_longer(cols = c(Lasso_CI_Size, OLS_CI_Size), 
               names_to = "Method", 
               values_to = "CI_Size")

# Rename methods for better readability
ci_data_lasso$Method <- ifelse(ci_data_lasso$Method == "Lasso_CI_Size", "Lasso", "OLS")

# Create the density plot
ggplot(ci_data_lasso, aes(x = CI_Size, fill = Method)) +
  geom_density(alpha = 0.5) +
  scale_x_log10() +
  labs(
    title = "Density Plot of Confidence Interval Sizes: Lasso vs. OLS",
    x = "Confidence Interval Size",
    y = "Density"
  ) +
  theme_minimal() +
...
# violin plot
ggplot(ci_data_lasso, aes(x = Method, y = CI_Size)) +
  geom_violin(fill = "lightblue") +
  geom_boxplot(width = 0.1, fill = "white") +
  theme_minimal() +
  labs(title = "Distribution of CI_Size by Group", x = "Group", y = "CI_Size")
