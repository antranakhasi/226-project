...
# Extract and identify statistically significant coefficients (p-value < 0.05)
significant_coeffs <- coefficients[coefficients[, 4] < 0.05, ]

# Print the statistically significant coefficients and their p-values
print("Statistically significant coefficients:")
print(significant_coeffs)

# Now, let's extract the names of the statistically significant variables
significant_variable_names <- rownames(significant_coeffs)
print("Significant variables:")
print(significant_variable_names)

```

### Subset of covariates model

```{r}
# Select a subset of covariates (as you did before)
...
df_subset$Revenue.Total <- NULL

# Create the formula for OLS regression
ols_formula <- as.formula(paste("Log_Revenue_Total ~", paste(subset_column_names[-6], collapse = " + ")))

# Train the OLS regression model
ols_model <- lm(ols_formula, data = df_subset)

# Print the summary of the OLS model
cat("OLS model coefficients summary:\n")
summary(ols_model)

# Extract and print the coefficients
cat("OLS model coefficients:\n")
print(coef(ols_model))

# Optional: Check the significance of each coefficient using the summary output
cat("OLS model coefficient significance:\n")
summary(ols_model)$coefficients[, 4]  # p-values of the coefficients
...
p_values <- ols_summary$coefficients[, 4]

# Set a significance threshold (e.g., 0.05)
significance_threshold <- 0.05

# Identify significant coefficients based on p-value
significant_coeffs <- names(p_values)[p_values < significance_threshold]

# Print the significant coefficients
cat("Significant coefficients based on p-value < 0.05:\n")
print(significant_coeffs)
...
ols_test_coefs <- coef(ols_test)

# Extract coefficients from training and holdout models
training_coefs <- as.matrix(coef(ols_train))
holdout_coefs <- as.matrix(ols_test_coefs)
