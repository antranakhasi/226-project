...
```{r fit model on the holdout set} 
...
# Create holdout_pred with only the specified columns from holdout
holdout_pred <- holdout[, column_names]

### Applying data transformations
# Applying log transformation to the Revenue.Total column
holdout_pred$Log_Revenue_Total <- log1p(holdout_pred$Revenue.Total)

# Prepare the holdout dataset
holdout_pred <- holdout[, column_names]
holdout_pred$Log_Revenue_Total <- log(holdout_pred$Revenue.Total)

# Prepare the data
X <- model.matrix(Log_Revenue_Total ~ ., data = holdout_pred)[, -1]  # Design matrix (exclude intercept)
y <- holdout_pred$Log_Revenue_Total  # Response variable

# Prepare the input matrices for glmnet
x <- model.matrix(Log_Revenue_Total ~ ., data = df_pred)[, -1]  # Remove intercept column
y <- df_pred$Log_Revenue_Total

# Fit the Ridge regression model with predefined lambda
ridge_model <- glmnet(x, y, alpha = 0, lambda = final_lambda_ridge, standardize = TRUE)
