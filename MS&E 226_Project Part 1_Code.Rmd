...
# load data
df <- read.csv("/Users/sydneyloh/OneDrive/Documents/Stanford/MS&E 226/Term Project/electricity.csv")
...
```{r Correlations, fig.show='hide', results='hide'}
...
```{r 5f subgroups in data, fig.show='hide', results='hide'}
...
columns_to_exclude <- c("Utility.Name", "Utility.Number", "Utility.State", 
                         "Revenue.Retail", "Revenue.Delivery", 
                         "Revenue.Resale", "Revenue.Adjustments", 
                         "Revenue.Transmission", "Revenue.Other", "Uses.Consumed") #Revenue subparts are linearly dependent on total revenue

# Create a new data frame without the excluded columns
# df_pred <- df[, !names(df) %in% columns_to_exclude]
